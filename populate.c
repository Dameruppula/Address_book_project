...
#include "project.h"
// Dummy contact data
void populateAddressBook(cont * contacts,int *size)
{
static cont dummyContacts[] = 
{
    ...
    for (int i = 0; i < numDummyContacts && *size < MAX_CONTACTS; ++i) 
    {
        contacts[*size] = dummyContacts[i];
        (*size)++;
     }
}